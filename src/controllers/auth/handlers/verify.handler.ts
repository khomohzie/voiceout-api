...
const sendEmail = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const msg = `<div style="text-align:center;">Use this code to verify your Shelter account
		...
		await transporter(email, "Shelter account - Email verification", msg)
			.then((data: any) => {
	...
	try {
		...
		if (!userCode) {
			return next(
				new CustomException(
					400,
					"Invalid OTP! Please request for another one"
				)
			);
		}
